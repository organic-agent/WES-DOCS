...
관리자 UI와 `/internal/admin/v1` API는 Tailnet 안에서만 제공한다. `autogroup:member`와 `autogroup:tagged`가 `tag:wes-admin:443`에 접근할 수 있고, 22·8080·8443 직접 접근은 막는다. Funnel을 사용하지 않으며 관리자 보안 그룹의 공개 ingress는 0이다.

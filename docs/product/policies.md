...
- 사진 바이트는 서버를 지나지 않는다. 브라우저가 presigned URL로 객체 저장소와 통신한 뒤 완료 API를 호출한다.
- 현재 업로드 URL 계약은 JPEG·PNG·WebP·HEIC/HEIF Content-Type, 배치 수와 파일명 길이를 검사한다. RAW 지원·형식별 최대 바이트 크기는 현재 보장에 넣지 않는다.
...
- 제품 서버는 작업 계약을 관리하고 외부 AI Lambda 3개가 모델을 실행한다. 업로드 완료와 분석·카테고리 생성은 별도 요청이다.
- 갤러리 안의 전체 세부폴더 사이로 사진을 이동할 수 있다. 같은 컨셉 이동은 반응을 보존하고 다른 컨셉 이동·미분류 전환은 이전 컨셉 반응을 제거한다.
...
- 별점은 공동 결과를 덮어쓰며 현재 서버는 제출 잠금·클라이언트 예상 버전을 검사하지 않는다. 선택 항목의 SUBMITTED 잠금과 구분한다.
...
- 사용자 알림은 저장·범위별 조회와 이메일/브라우저 수신 설정을 제공한다.
- 셀렉 제출 알림은 해당 작업공간 OWNER에게 저장한다. 외부 채널 발송 완료를 뜻하지 않는다.
- 이메일·브라우저 실제 발송 정책은 [미정 범위](scope.md)에 남아 있다.

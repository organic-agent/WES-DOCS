...
세 역할의 여정과, 제품을 관통하는 두 개의 상태 체인이다. 화면 단위의 전체 정보 구조는 [아키텍처 — Information](../architecture/information.md)에 있다.
...
- 대상 형식은 `jpg` `jpeg` `png` `raw` `heic`. 브라우저가 직접 그리지 못하는 RAW·HEIC는 고화질 표시용 변환본을 따로 만들고, **다운로드는 항상 원본**을 준다.
- 이 체인의 기술 구현은 [AI 파이프라인](../tech/ai-pipeline.md)에 있다.
...
- 같은 갤러리에서는 **한 번에 하나의 AI 셀렉 작업**만 돈다.
- 완료된 초안은 임의로 재생성하지 않고, 사용자가 편집한 결과를 자동으로 다시 채우지 않는다.
